...
import type { Directory, DirectoryWithTags } from "../types/models";
import { formatDirectoryList, success } from "../util/output";
import { canonical } from "../util/path";
    ...
    .input(
      z.object({
        ...
        tags: z
          .array(z.string())
          .optional()
          .describe("Comma-separated list of tags to apply"),
      }),
    )
    .mutation(({ ctx, input }) => {
      try {
  ...
  list: t.procedure
    ...
    .query(({ ctx, input }) => {
      try {
        ...
        // Enrich with tags
        const directoriesWithTags: DirectoryWithTags[] = directories.map(
          (dir) => ({
            ...dir,
